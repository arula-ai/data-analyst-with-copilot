...
**Use this before saving Phase 3 outputs and before the group debrief.**
...
Run through every item before saving your Phase 3 chart outputs and `[X]_cleaning_decisions.md`.
...
- [ ] Rates are calculated on valid rows only — not on all rows including nulls, sentinel values, or flagged invalids
...
### Representativeness
- [ ] Sample sizes are disclosed alongside all comparisons — "Institutional clients had the highest anomaly rate" must include the count of Institutional alerts
...
- [ ] No recommendation implies that removing a correlated factor will reduce anomaly without additional evidence
...
|-------|-------------------|-------------------|
| **Cherry-picking** | Reporting only the regions with high anomaly rates, ignoring regions where the pattern doesn't hold | Report all regions; note where the pattern is and isn't consistent |
...
| **Survivorship bias** | Analyzing only "investigation_complete = Yes" rows as if they represent all anomaly | Acknowledge that incomplete investigations may skew confirmed anomaly counts |
| **Causation from correlation** | "High prior_alerts_90d causes anomaly confirmation" | "Accounts with higher prior_alerts_90d had higher confirmed anomaly rates in Q4 2024" |
...
| **Operations Manager** | Finding + supporting stat + business implication | Raw DataFrames, statistical jargon, p-values without explanation |
...
1. Pick any number or finding in your output (e.g., "Velocity Check alerts had a 42% confirmed anomaly rate")
